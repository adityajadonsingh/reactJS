...
const App = () => {
  ...
  const generate = () => {
    ...
    if (input > 10 || input <= 0) {
      alert("It's out of range request... Please Enter between 1 - 10 !");
      return false;
    }
    const mapArray = paraData.slice(0, input);
    setContent(mapArray);
  };
    ...
    <>
      <div className="container">
        ...
        <div className="input-field">
          ...
          <input id="input" type="number" placeholder="0" />
          <button onClick={generate}>Generate</button>
        </div>
        <hr />
        <div className="container-para">
          {paraContent.map((item, idx) => (
            <div className="para">
              <span>{idx + 1}. </span>
              <p>{item}</p>
            </div>
